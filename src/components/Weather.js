...
function Weather() {
    ...
    const updateMode = (mode) => {
        if (mode === 0) {
            if (tempMode !== 0) { changeTempMode(0); changeTemp(Math.floor(((temp - 32) * 5) / 9)); }
        }
        else {
            if (tempMode !== 1) { changeTempMode(1); changeTemp(Math.round((temp * 9 / 5) + 32)); }
        }
    }
    ...
    const handleSearch = async (city) => {
        ...
        let tmp = false;
        myArray.forEach(ele => { if (ele === city) tmp = true; });

        if (tmp === false)
            updateMyArray(arr => [...arr, city]);
        ...
        try {
            ...
            axios.get(apiUrl + city + `&appid=${apiKey}`)
                .then((res) => {
                    ...
                    changeCity('No such city !'); changeTemp(0);
                    changeHumidity(0);
                    changeWindSpeed(0);
                });
        }
    ...
    return (
        <div className="card">
            <div className="search">
                <input type="text" placeholder="Enter city name" spellCheck={false} value={sVal} onChange={evt => updateVal(evt.target.value)} />
                <button><img src={require('./images/search.png')} alt="Search" onClick={() => { handleSearch(sVal) }} /></button>
            </div>
            ...
            <div className="weather">
                <img src={require(`${weatherIcon}`)} alt="Weather Icon" className="weather-icon" />
                <h1 className="temp">{temp}{tempMode === 0 ? '°C' : '°F'}</h1>
            ...
            <div className='temperature'>
                <button onClick={() => { updateMode(0) }} >Celcius</button>
                <button onClick={() => { updateMode(1) }}>Fahrenheit</button>
            </div>
        </div>
